...
const calculateNumber = require('./1-calcul.js'); // if tests live under ./test, change to ../1-calcul.js
...
describe('calculateNumber(type, a, b)', function () {
  ...
  describe('SUBTRACT', function () {
    ...
    it('handles negatives', function () {
      // JS rounding: Math.round(-1.5) === -1, Math.round(-3.5) === -3
      // So: -1 - (-3) = 2
  ...
  describe('DIVIDE', function () {
    ...
    it('rounds both then divides (various)', function () {
      ...
      assert.strictEqual(calculateNumber('DIVIDE', 3.1, 2.5), 1);     // 3 / 3
      assert.strictEqual(calculateNumber('DIVIDE', 0.4, 4.6), 0);     // 0 / 5
      assert.strictEqual(calculateNumber('DIVIDE', -2.5, 3.5), -0.5); // -2 / 4
      assert.strictEqual(calculateNumber('DIVIDE', -1.5, -3.5), 1/3); // -1 / -3
    });
    ...
    it('returns Error when rounded denominator is 0 (including negative zero)', function () {
      ...
      assert.strictEqual(calculateNumber('DIVIDE', 1.4, -0.5), 'Error');  // Math.round(-0.5) is -0
    });
