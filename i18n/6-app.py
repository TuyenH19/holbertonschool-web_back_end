#!/usr/bin/env python3
"""
Basic Flask + Flask-Babel app demonstrating mock locale selection.
"""
...
supported = app.config.get("LANGUAGES", [])
...
def get_locale() -> str:
    ...
    if req_locale in supported:
        return req_locale

    user = getattr(g, "user", None)
    if user and user.get("locale") in supported:
        return user["locale"]
